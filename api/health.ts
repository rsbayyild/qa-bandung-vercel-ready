...
export default {
  async fetch(request: Request) {
    ...
    const base = { ok: true, runtime: `node ${process.version}`, provider, model, ...runtime };

    if (url.searchParams.get("test") !== "provider") return Response.json(base);
    ...
    try {
      ...
      return Response.json({ ...base, ok: false, reachable: false, error: safeError(error) }, { status: 500 });
    }
  },
};
